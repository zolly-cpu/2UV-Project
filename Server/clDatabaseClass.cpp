...
clDatabaseClass::clDatabaseClass(QString paUserName, QString paPassword, QString paDatabaseName, QString paHostAdress, QString paPort, QString paDatabaseTech, vector<clDatabaseTable> paXMLDatabaseTable, vector<clDatabaseRoutine> paXMLDatabaseRoutine)
{
    ...
    QString loMessage = QString("");	
    initializeDatabases(loMessage);
}
...
bool clDatabaseClass::initializeDatabases(QString &paMessage)
{
    try
    {
        ...
        {
            case 1:
                    meDatabasePostgres = new clDatabasePostgres(getUserName(),getPassword(),getDataseName(),getHostAdress(),getPort(), meXMLDatabaseTable, meXMLDatabaseRoutine);
...
bool clDatabaseClass::getFromTableDatabaseGeneral(QString paTableName,QString paStartValue, QString paMaxValue, vector <QString> &paReturnId, vector <QString> &paReturnName, QString &paMessage)
{
    try
    {
        switch(getIDatabaseTech())
        {
            case 1:
                    return meDatabasePostgres->getFromTableDatabaseGeneral( paTableName, paStartValue, paMaxValue, paReturnId, paReturnName, paMessage);
                    break;
            case 2:
                    //meDatabaseMySql = new clDatabaseMySql(getUserName(),getPassword(),getDataseName(),getHostAdress(),getPort());
                    break;
